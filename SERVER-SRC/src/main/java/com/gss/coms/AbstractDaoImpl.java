...
public abstract class AbstractDaoImpl<Entity, ID extends Serializable> implements AbstractDao<Entity, ID> {
	...
	protected Session getSession(SessionVariable session) {
	...
	protected void closeSession(SessionVariable session)
	...
	private Criteria createEntityCriteria(SessionVariable session) {
		logger.info("createEntityCriteria");
		try {
			//return getSession().createCriteria(daoType);
			session.setSession(sessionFactory.openSession());
			return session.getSession().createCriteria(daoType);
